 ...
 * Fase 0 — Copia funções RPC do B1 → B2 automaticamente.
...
import { CLUSTER_ALL_RPCS } from '../../lib/supabaseCluster/tables.js';
...
const RPC_ORDER = CLUSTER_ALL_RPCS;
...
async function fetchFunctionDefs(client) {
  ...
  if (missing.length > 0) {
    ...
    console.warn(`⚠️ Funções não encontradas no B1 (serão ignoradas): ${missing.join(', ')}`);

  }



  return RPC_ORDER.filter((name) => byName.has(name)).map((name) => ({
